...
namespace Fvog
{
  TimerQueryAsync::TimerQueryAsync(uint32_t N, std::string name)
    : capacity_(N),
      name_(std::move(name))
  {
    assert(capacity_ > 0);

    vkCreateQueryPool(Fvog::GetDevice().device_,
    ...
    vkResetQueryPool(Fvog::GetDevice().device_, queryPool_, 0, N * 2);
  }
  ...
  TimerQueryAsync::~TimerQueryAsync()
  {
    if (queryPool_)
    {
      vkDestroyQueryPool(Fvog::GetDevice().device_, queryPool_, nullptr);
    }
  }
  ...
  void TimerQueryAsync::BeginZone(VkCommandBuffer commandBuffer)
  {
    ...
    if (count_ < capacity_)
    {
      vkResetQueryPool(Fvog::GetDevice().device_, queryPool_, start_, 1);
  ...
  void TimerQueryAsync::EndZone(VkCommandBuffer commandBuffer)
  {
    ...
    if (count_ < capacity_)
    {
      vkResetQueryPool(Fvog::GetDevice().device_, queryPool_, start_ + capacity_, 1);
  ...
  std::optional<uint64_t> TimerQueryAsync::PopTimestamp()
  {
    ...
    vkGetQueryPoolResults(Fvog::GetDevice().device_, queryPool_, index, 1, sizeof(startResult), &startResult, 0, flags);
    vkGetQueryPoolResults(Fvog::GetDevice().device_, queryPool_, index + capacity_, 1, sizeof(endResult), &endResult, 0, flags);
    ...
    count_--;
    const auto period = (uint64_t)Fvog::GetDevice().device_.physical_device.properties.limits.timestampPeriod;
